...
Scrapes job listings from WeWorkRemotely with fallback title logic.
...
class WeWorkRemotelyScraper(BaseScraper):
    ...
    async def scrape_jobs(self, search_params: Dict[str, Any]) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        try:
            ...
            logger.info(f"Scraping WeWorkRemotely: {search_url}")
    ...
    def _build_search_url(self, search_params: Dict[str, Any]) -> str:
        ...
        # Map search parameters to WeWorkRemotely format
        if search_params.get('keywords'):
            params['query'] = search_params['keywords']
            
        if search_params.get('location'):
            # WeWorkRemotely is primarily remote, but can filter by location
    ...
    async def _make_request(self, url: str) -> Optional[str]:
        """Make HTTP request to WeWorkRemotely"""
        try:
            async with self.session.get(url) as response:
                logger.debug(f"WeWorkRemotely request: {response.status} - {url}")
                
                if response.status == 200:
                    content = await response.text()
                    logger.debug(f"WeWorkRemotely response length: {len(content)}")
                    return content
                else:
                    logger.warning(f"WeWorkRemotely request failed: {response.status}")
                    return None
    ...
    async def _parse_job_listings(self, html_content: str, search_params: Dict[str, Any]) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        try:
            ...
            # Find job cards - WeWorkRemotely uses various selectors
            job_cards = soup.find_all(['div', 'article', 'li'], class_=lambda x: x and any(
                keyword in x.lower() for keyword in ['job', 'listing', 'card', 'item', 'post', 'feature']
            ))
            
            logger.debug(f"Found {len(job_cards)} job cards on WeWorkRemotely")
            ...
            logger.error(f"Error parsing WeWorkRemotely job listings: {e}")
    ...
    async def _extract_job_from_card(self, card, search_params: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            title_selectors = [
                ...
                'a[href*="/remote-jobs/"]', '[class*="title"]', '.job-link', '.feature'
            ]
            ...
            company_selectors = [
                ...
                '[class*="employer"]', 'a[href*="/companies/"]', '.author', '.company-name'
            ]
            ...
            # Extract location (WeWorkRemotely is primarily remote)
            location = 'Remote'  # Default to remote
            location_selectors = [
                '.location', '.city', '.country', '[class*="location"]',
                '[class*="city"]', '[class*="country"]', '.region'
            ]
            ...
            salary = None
            salary_selectors = [
                '.salary', '.compensation', '[class*="salary"]', '[class*="comp"]', '.rate', '.price'
            ]
            ...
            posted_date = None
            date_selectors = [
                '.date', '.posted', '.time', '[class*="date"]', '[class*="time"]', '.meta', '.timestamp'
            ]
            ...
            logger.debug(f"Extracted WeWorkRemotely job: {title} at {company}")
            return job_data
            
        except Exception as e:
            logger.debug(f"Error extracting WeWorkRemotely job data: {e}")
            return None
    ...
    async def _parse_job_details(self, html_content: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            desc_selectors = [
                ...
                '[class*="description"]', '[class*="content"]', '.entry-content', '.job-content'
            ]
            ...
            logger.error(f"Error parsing WeWorkRemotely job details: {e}")
            return None
