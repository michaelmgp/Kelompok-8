                    ...
                    <div className="space-y-3">
                      ...
                      <div className="space-y-2">
                        ...
                        <Button className="w-full" variant="outline" data-testid="button-ii-login-local" onClick={async () => {
                          try {
                            // Prefer the runtime page origin first, then environment override, then fallback.
                            const runtimeOrigin = (typeof window !== 'undefined' && window.location && window.location.origin) ? window.location.origin : '';
                            const origin = runtimeOrigin || process.env.NEXT_PUBLIC_FRONTEND_ORIGIN || 'http://localhost:6000';
                            const resp = await connectInternetIdentity({ origin });
                            if (resp.ok && resp.url) {
                              // Debug: log the actual URL we'll open so you can verify origin/fragment
                              console.debug('[Local II] opening URL:', resp.url);
