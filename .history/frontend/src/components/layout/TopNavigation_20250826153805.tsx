                    ...
                    <div className="space-y-3">
                      ...
                      <div className="space-y-2">
                        ...
                        <Button className="w-full" variant="outline" data-testid="button-ii-login-local" onClick={async () => {
                          try {
                            // Use server API to build a proper authorize URL that includes redirect
                            // so II will return to /dashboard on our frontend. no_redirect=1 returns
                            // the URL as JSON instead of performing a redirect.
                            const resp = await fetch('/api/ii?redirect=/dashboard&no_redirect=1');
                            const data = await resp.json();
                            if (data && data.url) {
                              console.debug('[Local II] opening URL:', data.url);
                              window.open(data.url, 'icp_local', 'width=600,height=800');
                            } else {
                              alert('Local II not available: ' + (data && data.error ? data.error : 'unknown'));
                            }
                          } catch (e) {
